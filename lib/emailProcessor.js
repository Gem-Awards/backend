...
const { getUnreadMessages, markAsRead, tagCategory, replyToMessage, createDraftReply } = require('./emailInbox');
const {
  createConversation,
  saveMessage,
...
async function processOneEmail(message) {
  ...
  try {
    const replyMode = await getSetting('email_reply_mode', 'send');

    if (replyMode === 'draft') {
      // Prepare a draft for a real person to review, edit if needed, and
      // send themselves - nothing goes to the customer automatically.
      await createDraftReply(message.id, replyText);
      await markAsRead(message.id).catch(() => {});
      await tagCategory(
        message.id,
        needsFollowup ? 'AI: Draft Ready (Needs Follow-up)' : 'AI: Draft Ready'
      ).catch(() => {});
      ...
      await saveMessage({
        ...
        conversationId,
        sender: 'ai',
        content: `[Draft prepared for review - not sent automatically]\n\n${replyText}`,
        escalate: true,
        conversationStatus: 'escalated',
      });
      return;
